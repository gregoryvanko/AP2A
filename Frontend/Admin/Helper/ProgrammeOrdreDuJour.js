class ProgrammeOredredujour{
    ...
    Show(Type = "", Programme = null){
        ...
        switch (Type) {
            ...
            case ProgrammeOredredujour.ConstBandeau():
                this.ShowBandeau(Programme)
                break;
            case ProgrammeOredredujour.ConstAugmSalaire():
                this.ShowAugmSalaire(Programme)
                break;
            default:
                alert("Type of Programme not found: " + this._TypeTenue)
                break;
        }
    }
    ...
    ShowMorceauArchitecture(DataMorceauArchitecture = null){
        ...
        conteneur.appendChild(NanoXBuild.Button("Ajouter un orateur", this.BuildViewAddModifyIntervenant.bind(this, null, true, true), null, "Button MarginButton Text", "width: 15rem; margin-left: auto; margin-right: auto;")) 
    ...
    ShowDossier(DataDossier = null){
        ...
        this.BuildListofIntervenant()
        // Boutton Add Intervenant
        conteneur.appendChild(NanoXBuild.Button("Ajouter un profane", this.BuildViewAddModifyIntervenant.bind(this, null, false, false), null, "Button MarginButton Text", "width: 15rem; margin-left: auto; margin-right: auto;")) 
    ...
    /* Bandeau */
    ShowBandeau(DataBandeau = null){
        if (DataBandeau == null){ DataBandeau = {Valide: false, ListeIntervenant: []}}
        this._ListeIntervenant = DataBandeau.ListeIntervenant
        ...
        this._DivApp.appendChild(conteneur)
        // Titre
        conteneur.appendChild(NanoXBuild.DivText("Bandeau", null, "Titre MarginTitre", "margin-bottom: 1rem; margin-top: 1rem; text-align: center;"))
        // Liste Profane
        conteneur.appendChild(NanoXBuild.DivText("Liste des profanes", null, "SousTitre", "width:100%; margin-bottom: 0.5rem; margin-top: 2rem;"))
        let conteneurIntervenant = NanoXBuild.DivFlexColumn(this._IdconteneurIntervenant, null, "width:100%;")
        conteneur.appendChild(conteneurIntervenant)
        ...
        // Valide
        conteneur.appendChild(NanoXBuild.DivText("Validation", null, "SousTitre", "width:100%; margin-bottom: 0.5rem; margin-top: 2rem;"))
        conteneur.appendChild(UiComponent.InputWithToogle("Validé:", "InputValide", DataBandeau.Valide))

        // Save Cancel boutton
        let ConteneurAction = NanoXBuild.DivFlexRowSpaceEvenly(null, null, "width: 100%; margin-top: 1rem;")
        ConteneurAction.appendChild(NanoXBuild.Button("Save", this.ClickSaveBandeau.bind(this), null, "Button MarginButton Text", "width: 6rem;"))
        ConteneurAction.appendChild(NanoXBuild.Button("Cancel", this.ClickCancel.bind(this), null, "Button MarginButton Text", "width: 6rem;"))
        conteneur.appendChild(ConteneurAction)
    }
    ...
    ShowAugmSalaire(DataAugmSalaire = null){
        if (DataAugmSalaire == null){ DataAugmSalaire = {Valide: false, ListeIntervenant: []}}
        this._ListeIntervenant = DataAugmSalaire.ListeIntervenant
        // Clear view
        this._DivApp.innerHTML=""
        // Clear liste of  updatede Intervenant
        this._IndexOfUpdatedIntervenant = null
        ...
        this._DivApp.appendChild(conteneur)
        // Titre
        conteneur.appendChild(NanoXBuild.DivText("Augmentation de salaire", null, "Titre MarginTitre", "margin-bottom: 1rem; margin-top: 1rem; text-align: center;"))
        // Liste Intervenant
        conteneur.appendChild(NanoXBuild.DivText("Liste des orateurs", null, "SousTitre", "width:100%; margin-bottom: 0.5rem; margin-top: 2rem;"))
        let conteneurIntervenant = NanoXBuild.DivFlexColumn(this._IdconteneurIntervenant, null, "width:100%;")
        conteneur.appendChild(conteneurIntervenant)
        ...
        this.BuildListofIntervenant()
        // Boutton Add Intervenant
        conteneur.appendChild(NanoXBuild.Button("Ajouter un orateur", this.BuildViewAddModifyIntervenant.bind(this, null, true, false), null, "Button MarginButton Text", "width: 15rem; margin-left: auto; margin-right: auto;")) 
        
        // Valide
        conteneur.appendChild(NanoXBuild.DivText("Validation", null, "SousTitre", "width:100%; margin-bottom: 0.5rem; margin-top: 2rem;"))
        conteneur.appendChild(UiComponent.InputWithToogle("Validé:", "InputValide", DataAugmSalaire.Valide))

        // Save Cancel boutton
        let ConteneurAction = NanoXBuild.DivFlexRowSpaceEvenly(null, null, "width: 100%; margin-top: 1rem;")
        ConteneurAction.appendChild(NanoXBuild.Button("Save", this.ClickSaveAugmSalaire.bind(this), null, "Button MarginButton Text", "width: 6rem;"))
        ConteneurAction.appendChild(NanoXBuild.Button("Cancel", this.ClickCancel.bind(this), null, "Button MarginButton Text", "width: 6rem;"))
        conteneur.appendChild(ConteneurAction)
    }

    ClickSaveAugmSalaire(){
        if(this._ListeIntervenant.length == 0){
            // Popup error
            let content = NanoXBuild.DivText("Erreur: ajouter un orateur", null, "Text", "color: red; text-align: center;")
            NanoXBuild.PopupCreate(content)
        } else {
            let Data = {
                Type: this._TypeTenue,
    ...
    BuildListofIntervenant(){
        ...
        this._ListeIntervenant.forEach(Intervenant => {
            ...
            if(Intervenant.Macon && Intervenant.ChoixLoge){
    ...
    BuildViewAddModifyIntervenant(Intervenant = null, Macon = true, ChoixLoge = true){
        if (Intervenant == null){ 
            Intervenant = {Nom:"", Prenom:"", Macon: Macon, ChoixLoge : ChoixLoge, Loge:"", DeAtelier: true}
        } else {
            this._IndexOfUpdatedIntervenant = this._ListeIntervenant.indexOf(Intervenant)
            Macon = Intervenant.Macon
            ChoixLoge = Intervenant.ChoixLoge
        }
        // Conteneur
        ...
        // Save Cancel boutton
        const ListOfButton = [{Titre: "Save", Action: this.SaveIntervenant.bind(this, Macon, ChoixLoge), Id: null}, {Titre: "Cancel", Action: NanoXBuild.PopupDelete, Id: null}]
        // Creation du popup
    ...
    SaveIntervenant(Macon = true, ChoixLoge = true){
        ...
        let Intervenant = (Macon)? {
            ...
            Macon: Macon,
            ChoixLoge: ChoixLoge,
            Loge: (ChoixLoge)? document.getElementById("InputIntervenantLoge").value : true, 
            DeAtelier: (ChoixLoge)? document.getElementById("DeAtelier").checked : ""
        }: {
            Prenom: document.getElementById("InputIntervenantPrenom").value,
            Nom: document.getElementById("InputIntervenantNom").value,
            Macon : Macon,
